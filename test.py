...
# # 배경음악
# sound = pygame.mixer.Sound(file='25. Ultimate Battle IN-GAME.mp3')
# bitrate = 250000  # 250 Kbps
# bytes_per_second = bitrate // 8

# # 2 to 4 seconds corresponds to:
# start_byte = 2 * bytes_per_second
# end_byte = 4 * bytes_per_second

# # Get the raw sound data and slice the portion
# raw_array = sound.get_raw()
# raw_segment = raw_array[start_byte:end_byte]

# # Create a new sound object using the sliced data
# cut_sound = pygame.mixer.Sound(buffer=raw_segment)

# # Play the sound on loop (-1 means infinite loop)
# cut_sound.play(loops=-1)
...
peanut = pygame.image.load("peanut.png").convert_alpha()
peanut = pygame.transform.scale(peanut, ( 100, 100 ))
doubleflower = pygame.image.load("doubleflower.webp").convert_alpha()
doubleflower = pygame.transform.scale(doubleflower, ( 100, 100 ))
...
font_a = pygame.font.SysFont("arial", 25,  True, True)
        ...
        if time.time() - self.last_shot_time >= self.fire_rate:
            if self.plant["Name"] == "MEa":
                ...
                if map[self.col][self.row + 1] == 0:
                    ...
                    for zombie in zombies[:]:
                        if zombie.x - self.x <= 220 and zombie.y == self.y: # y는 나중에 수정
                            zombies.remove(zombie)
                            bigchomp.play()
                self.last_shot_time = time.time()
            ...
            elif self.plant["Name"] == "Doubleflower":
                suns.append(Sun(self.x - 10, self.y + 50, 25, False))
                suns.append(Sun(self.x + 70, self.y + 50, 25, False))
                self.last_shot_time = time.time()
            elif self.plant["Name"] == "PeaNut":
                pass
...
class Inventory:
    ...
    def update(self, SCREEN, LastRate):
        ...
        Text = font_a.render(f"{self.Cost}", True, black)
        screen.blit(Text, (self.x + self.width - 30, self.y + self.height - 30))

# test_a = Inventory(1100, 500, inv_width, inv_height, inventory[1])

test_inv = [Inventory(1100, 10 +i * inv_margin, inv_width, inv_height, inventory[i]) for i in range(len(inventory))]
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for index, inv in enumerate(test_inv):
                if (inv.rect.collidepoint(mouse_x, mouse_y)): # demo
                    print("오 정재헌씨, 정답입니다.")
                
                    if sunAmount >= inventory[index]["Cost"]:
                        if time.time() - inventory[index]["LastRate"] >= inventory[index]["PlantRate"]:
                            if Selected == inventory[index]:
                                Selected = None
                                break
    ...
    #인벤토리 표시
    for index, inv in enumerate(test_inv):
        inv.update(screen, inventory[index]["LastRate"])
    # test_a.update(screen, inventory[1]["LastRate"])
